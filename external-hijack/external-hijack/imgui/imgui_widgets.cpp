...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool active)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (active) {
        ...
        min.y = max.y;
        GetWindowDrawList()->AddLine(min, max, ImColor(255, 255, 255, 255), 1.0f);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3.f, 3.f), check_bb.Max - ImVec2(3.f, 3.f), check_col, style.FrameRounding - 0.5f);
    }
...
bool ImGui::Checkbox(const char* label, bool* v, const std::function<void()>& on_click)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Items_SingleStringGetter(void* data, int idx, const char** out_text)
{
...
bool ImGui::RangeSliderBehavior(ImRect frame_bb, ImGuiID id, float* v1, float* v2, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiStyle& style = g.Style;
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool* rainbow, int* rainbow_speed, ImGuiColorEditFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            if (*rainbow) {
                ...
                ImGui::InputInt("Speed", rainbow_speed);
                ImGui::PopItemWidth();
            }
            EndPopup();
        }
