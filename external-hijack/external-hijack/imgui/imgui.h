...
struct ImVec4
{
    float x, y, z, w{}; //bool is_rainbow{}; float rainbow_speed{};

    constexpr ImVec4() : x(0.0f), y(0.0f), z(0.0f), w(0.0f) { }
    constexpr ImVec4(float _x, float _y, float _z, float _w)  : x(_x), y(_y), z(_z), w(_w) { }

   //ImVec4(float _x, float _y, float _z, float _w, bool _is_rainbow, float _rainbow_speed) : x(_x), y(_y), z(_z), w(_w), is_rainbow(_is_rainbow), rainbow_speed(_rainbow_speed) {
   //    //this->x = _x;
   //    //this->y = _y;
   //    //this->z = _z;
   //    //this->is_rainbow = _is_rainbow;
   //    //this->rainbow_speed = _rainbow_speed;
   //}

    //constexpr ImVec4() : x(0.0f), y(0.0f), z(0.0f), w(0.0f), is_rainbow(false), rainbow_speed(1.f){ }
    //constexpr ImVec4(float _x, float _y, float _z, float _w, bool _is_rainbow, float _rainbow_speed)  : x(_x), y(_y), z(_z), w(_w), is_rainbow(_is_rainbow), rainbow_speed(_rainbow_speed){ }
...
namespace ImGui
{
    ...
    IMGUI_API bool          Checkbox(const char* label, bool* v, const std::function<void()>& on_click);
    ...
    IMGUI_API bool          Items_SingleStringGetter(void* data, int idx, const char** out_text);
    ...
    IMGUI_API bool          RangeSliderFloat(const char* label, float* v1, float* v2, float v_min, float v_max, const char* display_format, float power);
    ...
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], bool* rainbow, int* rainbow_speed, ImGuiColorEditFlags flags = 0);
    // Widgets: Trees
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_DaddyPlzRoundBottomOnly= 1 << 29,  // round da bottam window cornaz onleh
...
struct ImColor
{
    ...
    inline const ImVec4 glow_color_bright() { return { Value.x * 3.f, Value.y * 3.f, Value.z * 3.f, Value.w }; };
    inline const ImVec4 glow_color() { return { Value.x, Value.y, Value.z, Value.w }; };
...
struct ImColorOrRainbow {
    ImVec4          Value;
    bool rainbow = false;
    static inline ImColor* rainbow_color = nullptr;

    constexpr ImColorOrRainbow() { }
    constexpr ImColorOrRainbow(float r, float g, float b, float a = 1.0f) : Value(r, g, b, a) { }
    constexpr ImColorOrRainbow(float r, float g, float b, float a, bool rr) : Value(r, g, b, a), rainbow(rr) { }
    constexpr ImColorOrRainbow(const ImVec4& col) : Value(col) {}
    constexpr ImColorOrRainbow(const ImColor& col) : Value(col.Value) {}
    ImColorOrRainbow(int r, int g, int b, int a = 255) { float sc = 1.0f / 255.0f; Value.x = (float)r * sc; Value.y = (float)g * sc; Value.z = (float)b * sc; Value.w = (float)a * sc; }
    ImColorOrRainbow(ImU32 rgba) { float sc = 1.0f / 255.0f; Value.x = (float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc; Value.y = (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc; Value.z = (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc; Value.w = (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc; }
    inline operator ImU32() const { return ImGui::ColorConvertFloat4ToU32(rainbow ? rainbow_color->Value : Value); }
    inline operator ImVec4() const { return rainbow ? rainbow_color->Value : Value; }
    inline operator ImColor() const { return rainbow ? rainbow_color->Value : Value; }
    inline const ImVec4 val() { return (rainbow ? (ImVec4(rainbow_color->Value.x, rainbow_color->Value.y, rainbow_color->Value.z, Value.w)) : Value); };
    inline const ImVec4 glow_color_bright() { ImVec4 val_new = val(); return { val_new.x * 3.f, val_new.y * 3.f, val_new.z * 3.f, val_new.w }; };
    inline const ImVec4 glow_color() { ImVec4 val_new = val(); return { val_new.x, val_new.y, val_new.z, val_new.w }; };
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddTextGradient(const ImVec2& pos, ImU32 col, ImU32 col2, const char* text_begin, const char* text_end = NULL);
